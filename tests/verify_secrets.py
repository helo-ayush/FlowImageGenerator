...
def verify_all_secrets() -> bool:
    ...
    storage_path_env = os.environ.get("STORAGE_STATE_PATH", "storage_state.json")
    storage_file = Path(storage_path_env)
    if not storage_file.is_absolute() and not storage_file.exists():
        storage_file = ROOT_DIR / storage_path_env
